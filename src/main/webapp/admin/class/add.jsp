...
<form action="${pageContext.request.contextPath}/admin/class" method="post">
    ...
    <label>班名：<input type="text" name="jxbmc" required /></label>
    <label>课程编号：<input type="text" name="kcbh" required /></label>
    <label>授课教师：
        <select name="jsbh" required>
            <option value="">请选择教师</option>
            <c:forEach var="t" items="${teacherList}">
                <option value="${t.jsbh}">${t.jsbh} - ${t.jsxmc}</option>
            </c:forEach>
        </select>
    </label>
    <label>上课时间：<input type="text" name="sksj" /></label>
    <label>上课地点：<input type="text" name="skdd" /></label>
    <label>选定人数：<input type="text" name="xdrs" value="0" /></label>
